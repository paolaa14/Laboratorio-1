...
• Función de probabilidad; consiste en una aproximación de forma teórica que enseña como se distribuyen los datos en términos generales ( en nuestro caso se observa como una línea curva continua roja).

3. En tercer lugar, se importaron las bibliotecas, como la ¨numpy¨ para relizar operaciones matematicas, o la ¨matplotlib.pyplot¨ para poder realizar los graficos que en este caso son histogramas, para continuar se implemento una parte en el código para leer la señal del documento tomado de physionet, que en el codigo es ¨wfdb.rdrecord("emg_healthy")¨, que va a leer basicamente el archivo de emg del physionet y lo guarda en una variable que le pusimos signal, después se calculó la media de la señal con ¨suma_informacion += valor_informacion ( que basicamente suma cada valor), luego "num_informacion = len(valores)" (que es el número total de muestras) y con eso se calcula la media que es : "media = suma_informacion / num_informacion ", posterior a esto se hizo algo similar para la desviación estándar pero calculandolo así: "diferencia = valor_informacion - media " (que es la  diferencia respecto a la media) y ahora de hace "diferencia_al_cuadrado += diferencia ** 2 " (que es la suma de los cuadrados de las diferencias) y recordando que la desviación estandar es la raiz cuadrada de la varianza, calculamos la varianza también : "varianza = diferencia_al_cuadrado / num_informacion " ( para recordar la varianza mide que tanto varían los datos en promedio) y para llegar al cálculo de la desviación estandar usamos: "desviacion_estandar = math.sqrt(varianza)"  ( Se obtiene la raíz cuadrada de la varianza), ahora para el coficiente de variación, se obtiene diviendo la desviación estándar por la media y multiplicnado por 100 para mostrarlo en términos de porcentaje, lo cual se refleja en el codigo,  y para la función de probabilidad para tener un menor análisis de esta, llamamos el archivo de funcioneslargas y codificamos los cálculos, la para luego en la consola de spyder observar lo mencionado previamente. Posterior, creamos un histograma , recalcando que en el eje “y” se encuentra la frecuencia relativa (repeticiones cada valor) por que los valores han sido normalizados , y en el eje “x” los valores de la amplitud (que tan alto llega cada barra) en voltaJe (mV) .Para continuar, después de crear el histograma hicimos la curva de densidad en forma de gráfico que nos enseña el centro, la extensión y la forma de los datos de esta señal. Todo esto lo evidenciamos en el archivo llamado funcioneslargas.
